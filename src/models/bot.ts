...
export async function bot(client: Client, message: WAWebJS.Message, step = false): Promise<void> {
    ...
    if (temp.level == 1) {
        ...
        temp.level = 2;
        temp = await cache(chatId, true, temp.level, temp.step, temp.answer, answer, null, temp.dateOfBirth, temp.name, temp.postalCodeId, temp.gender, temp.answerDetailId, temp.projectId, temp.email, temp.city, temp.urbanVillage, temp.province, temp.districts, temp.address, temp.userId, temp.messageId, temp.postalCode, temp.provinceData, temp.cityData, temp.districtsData, temp.urbanVillageData);
    }
            ...
            switch (previousData.question_type) {
                case "choice":
                    ...
                    if (!["buttons_response", "list_response"].includes(messageType) || !idOptions.includes(text.id) || !titleOptions.includes(text.title)) {
                        ...
                        await cache(chatId, true, temp.level, temp.step, temp.answer, answer, {}, temp.dateOfBirth, temp.name, temp.postalCodeId, temp.gender, temp.answerDetailId, temp.projectId, temp.email, temp.city, temp.urbanVillage, temp.province, temp.districts, temp.address, temp.userId, temp.messageId, temp.postalCode, temp.provinceData, temp.cityData, temp.districtsData, temp.urbanVillageData);
    ...
    switch (data.question_type) {
        ...
        case "text":
            ...
            await cache(chatId, true, temp.level, temp.step, temp.answer, answer, data, temp.dateOfBirth, temp.name, temp.postalCodeId, temp.gender, temp.answerDetailId, temp.projectId, temp.email, temp.city, temp.urbanVillage, temp.province, temp.districts, temp.address, temp.userId, temp.messageId, temp.postalCode, temp.provinceData, temp.cityData, temp.districtsData, temp.urbanVillageData);
            return bot(client, message, true);
